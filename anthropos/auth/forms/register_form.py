...
from anthropos.lib import CleanString, OnlyCharsValidator, DataRequiredImproved


class RegistrationForm(FlaskForm):
    username = StringField(label='Никнейм', validators=[Length(min=2, max=50), DataRequired()])
    first_name = StringField(label='Имя', validators=[Length(min=2, max=50), DataRequired(), CleanString(), OnlyCharsValidator()])
    last_name = StringField(label='Фамилия', validators=[Length(min=2, max=50), DataRequired(), CleanString(), OnlyCharsValidator()])
    middle_name = StringField(label='Отчество', validators=[Length(min=0, max=50), CleanString(), OnlyCharsValidator()])
